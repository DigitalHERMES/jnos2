...
/*
 * 09Aug2021, Maiko (VE4KLM), PERMANENT DEFINE !!!
 * New K> and K< allowing delete message by call.
 */

#define	J2KILLBYCALL	/* New K> and K< allowing delete message by call */

#define	ORIGINAL_JNOS111F	/* also PERMANENT for now, reverting K6FSH 'fix' for L< and L> bbs commands */
...
#ifdef	J2KILLBYCALL
		/*
		 * 04Aug2021, Maiko (VE4KLM), Trying to minimize FILE I/O in bmutil.c ?
		 *
		 * NOTE : we need to free() these values when we're done with them !!!
		 */
		cmsg->from = j2strdup (ind.from);
		cmsg->to = j2strdup (ind.to);
#endif
...
#ifdef	ORIGINAL_JNOS111F
		/*
		 * 09Aug2021, Maiko (VE4KLM), Numerous people have complained that
		 * the L> and L< are no longer working and have not for quite some
		 * time, looks like I should have paid more attention to reviewing
		 * the 2010 mods. These are not necessary, since the code further
		 * down takes care of any atoi() failures due to the strings.
		 *
		 * Compliance to documentation was taken too literally I think :(
		 *
		 */
			start = atoi (argv[1]);
		if (argc > 2)
...
#ifdef	ORIGINAL_JNOS111F
		/*
		 * 09Aug2021, Maiko (VE4KLM), See previous comments (above), and this one
		 * is just plain wrong, you'll never get to the string search code further
		 * down, so we are going to go back to the original JNOS functionality !
		 */
...
#ifdef	J2KILLBYCALL_NOTYET
			/* 04Aug2021, Maiko, don't need index file for this now */
                if(strstr(cmsg->to,argv[searchopt]) == NULL)
#else
...
#ifdef	J2KILLBYCALL_NOTYET
			/* 04Aug2021, Maiko, don't need index file for this now */
                if(strstr(cmsg->from,argv[searchopt]) == NULL)
#else
...
/*
 * 04Aug2021, Maiko (VE4KLM), Need a function that checks
 * if a character is part of a string, like the char version
 * of the strpbrk() function, can't seem to find one I like.
 */
int j2chrinstr (char item, char *accept)
{
	while (accept && *accept)
	{
		if (item == *accept)
...
void *p;
{
    ...
    char *tmpbuf;
#ifdef	J2KILLBYCALL
	char *killcall;		/* 04Aug2021, Maiko (VE4KLM), Kill FROM or TO call, Requested by N2NOV */
#endif
	...
	/*
	 * 04Aug2021, Maiko (VE4KLM), Requested by N2NOV (Charles Hargrove)
	 *
	 *   KH - kill held message
	 *   K< - kill from call
	 *   K> - kill to call
	 *
	 * 09Aug2021, Maiko (my idea) - KR to kill the READ flag on ALL messages
	 *
	 * These call based ones are more of a challenge, we need to read
	 * the messages to get the information, they're not part of 'let'
	 * structure, they are contained in the index file, but wait, we
	 * can add the mailindex '*to' and '*from' info to 'struct let',
	 * yes, that will work, do that in readnotes() function !
	 *
	 * Adding these to 'struct let' might actually cut down on FILE I/O
	 * in listnotes() when people are doing 'L>' and 'L<', that's good,
	 * so likely going to be doing some mods in that function as well.
	 */

	else if (j2chrinstr (m->stype, "ARMH<>"))	/* 04Aug2021, Maiko, new function to avoid all those || statements */
	{
		if (m->stype == '<' || m->stype == '>')
		{
#ifdef	J2KILLBYCALL
			if (argc != 2)
       			tprintf ("you need to specify the callsign\n");
			else
				killcall = j2strdup (argv[1]);

		log (-1, "killcall [%s]", killcall);
#else
			tprintf ("kill by call not included in this compile\n");
			killcall = (char*)0;
#endif
		}
        msg=0;
        for(i=1,cmsg=&m->mbox[1];i<=m->nmsgs;i++,cmsg++) {
		log (-1, "cmsg to [%s] from [%s]", cmsg->to, cmsg->from);
	    if (m->stype == 'R')				/* set ALL messages to UNREAD - Maiko 09Aug2021 */
              cmsg->status &= ~BM_READ, msg++;
	    else
            if(m->stype == 'A' ||
               (m->stype == '>' && !strcasecmp (cmsg->to, killcall)) ||  	/* found a matching TO msg - 04Aug2021 */
               (m->stype == '<' && !strcasecmp (cmsg->from, killcall)) ||	/* found a matching FROM msg - 04Aug2021 */
               (m->stype == 'H' && (cmsg->status & BM_HOLD)) ||  			/* found a held msg - 04Aug2021 */
