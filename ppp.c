   ...
   switch (type)
   {
      ...
      case 1:
         ...
         if (dup_p (&tbp, bp, 0, len) != len)
            log (dev, "ppp_putchar - dup_p failed");

         else
         {
             pullup (&tbp, iobuf, len);

             if (j2send (dev, iobuf, len, 0) < 1)
               log (dev, "ppp_putchar - write errno %d", errno);

             // else log (dev, "sent %d bytes", len);
...
struct mbuf *bp;
{
	 ...
	 */
	int ppp_vara = (strnicmp (ifp->name, "vara", 4) == 0);
