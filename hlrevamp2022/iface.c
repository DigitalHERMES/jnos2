...
static int ifrxonly __ARGS((int argc,char *argv[],void *p));
#endif
...
#ifdef KLM_HLREVAMP_2022
/*
 * 09Mar2022, Maiko (VE4LM), If I am going to be switching frequencies
 * on my RPR port, I need the ability to prevent TX on bands for which
 * my antenna is not cut or tuned for. Having this 'soft option' means
 * there is no need to disconnect PTT cables or whatever.
 */
    { "rxonly",               ifrxonly,       0,      1,      NULLCHAR },
#endif
...
static int ifrxonly (int argc, char **argv, void *p)
{
	struct iface *ifp = p;

	/*
	 * It would appear there is room for more 'flags' in the
	 * struct iface 'configuration flags', so I have decided
	 * to define a new IFACE_RXONLY flag to prevent any TX.
	 */

	ifp->flags ^= IFACE_RXONLY;	/* just use toggling */

	tprintf ("note : this command is a toggle !\n");

	if (ifp->flags & IFACE_RXONLY)
		tprintf ("interface \"%s\" now rx only\n", ifp->name);
	else
		tprintf ("interface \"%s\" back to tx and rx\n", ifp->name);
