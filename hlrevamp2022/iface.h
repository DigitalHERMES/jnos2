...
#define	IFACE_RXONLY   65636 /* Disable TX, 09Mar2022, Maiko (VE4KLM) */
#endif
...
#define IF_TRACE_POLLS  0x4000  /* Include polls in pkiss output trace */
	...
	/*
	 * 04Mar2022, Maiko (VE4KLM), new concept, heard lists are just groups
	 * of callsigns now, no more references to iface, so now we can have as
	 * many groups of calls as we want, and associate any group to any iface
	 * at any time. For instance what if I want to change freq or the band on
	 * the only RPR port I am running, and keep calls heard on different freq
	 * or band separate from each other, put them in their own heard list ?
	 *
	 * One massive link list for ALL calls on all ifaces makes no sense, and
	 * is very inefficient when the heard lists get very large. So instead of
	 * going through one massive link list, we can now point direct to a link
	 * list of calls for that one interface only - more flexibile this way.
	 *
	 * Make this a void ptr, since it could be struct *Lq, *Ld, or *Lv
	 *
	   void *hlptr;
	 *
	 * 05Mar2022, NOPE, stick with original structs, need 3 of them
	 */
