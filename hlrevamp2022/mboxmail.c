...
void j2filesentmsg (struct mbx *m, char *ffrom)
{
	char buf[LINELEN];
	FILE *fp;
	time_t t;
	...
	/* Now using <callsign>_s.txt, so it becomes just another area */
	sprintf (buf,"%s/%s_s.txt", Mailspool, m->name);

	if ((fp = fopen (buf, APPEND_TEXT)) == NULLFILE)
	{
		log (m->user, "unable to open [%s] - sent not saved", buf);

		tprintf ("unable to open [%s] - sent not saved\n", buf);
	}
	else
	{
		/*
		 * Make sure we write From and Received to be consistent
		 * with the general mailbox format, that way when we can
		 * read the *.snt file just like *.txt file - later on.
		 *  (will probably have to tweak these 2 lines later)
		 *
	`	 * 20May2022, Maiko (VE4KLM), Actually forget that idea !
		 * NOW just save to <callsign>_s.txt, that way it becomes just
		 * another area you can change into, and you instantly have
		 * to the messages like you would your own area, R, V, etc.
		 */

		time (&t);

		fprintf (fp, "From %s %s", ffrom, ctime(&t));

		fprintf (fp, "%sby %s with SMTP\n\tid AA%ld ; %s",
			Hdrs[RECEIVED], Hostname, get_msgid(), ptime(&t));

		//fprintf (fp, "%sfrom %s ; %s", Hdrs[RECEIVED], Hostname, ptime (&t));

		fseek (m->tfile, 0L, 0);

		while ((cnt = fread (buf, 1, sizeof(buf), m->tfile)) > 0)
			fwrite (buf, 1, cnt, fp);

		fputs ("\n", fp);	/* separate entries with a blank line */

		fclose (fp);
	}
}

#endif	/* end of J2_FILESENTMSGS */
